...
void kernel_xy(uint32_t x, uint32_t y, 
	...
	const uint32_t *world_properties,
	...
	for(unsigned t=0;t<n;t++){
		for(unsigned y=0;y<h;y++){
			for(unsigned x=0;x<w;x++){
				kernel_xy(x,y,w,outer,inner, (const uint32_t*) &world.properties[0], &world.state[0], &buffer[0]);
			}  // end of for(x...
		} // end of for(y...
...
int main(int argc, char *argv[])
{
	...
	try{
		...
		hpce::bcs10::StepWorldV2Lambda(world, dt, n);
